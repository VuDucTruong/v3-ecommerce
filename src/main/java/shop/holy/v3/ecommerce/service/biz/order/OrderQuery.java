...
public class OrderQuery {
    ...
    public CompletableFuture<ResponseOrder> findById(long id, boolean deleted) {
        ...
        return CompletableFuture.allOf(futureOrder, futureDetails).thenApply(v -> {
            List<OrderDetail> details = futureDetails.join();
            Order order = futureOrder.join();
            return orderMapper.fromEntityToResponse_InDetail(order, details);
        });
    }
